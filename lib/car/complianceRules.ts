...
export function normalizeVehicleComplianceDates(input: VehicleComplianceDates): VehicleComplianceDates {
...
export function vehicleComplianceState(input: VehicleComplianceDates, today: string): 'COMPLETE' | 'EXPIRING' | 'EXPIRED' | 'INCOMPLETE' {
  ...
  return values.some((value) => value <= warning.toISOString().slice(0, 10)) ? 'EXPIRING' : 'COMPLETE';
}
