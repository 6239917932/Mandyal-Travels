...
export function normalizeVehicleMaintenance(input: VehicleMaintenanceInput): VehicleMaintenanceInput {
  ...
  const start = DATE_PATTERN.test(input.startDate) ? Date.parse(`${input.startDate}T00:00:00Z`) : NaN;
  const end = DATE_PATTERN.test(input.endDate) ? Date.parse(`${input.endDate}T00:00:00Z`) : NaN;
  if (!category || !status || !description || !Number.isFinite(start) || !Number.isFinite(end) || end < start) {
    throw new Error('Enter a valid maintenance category, status, date range, and description.');
  }
  if (input.costAmount !== undefined && (!Number.isInteger(input.costAmount) || input.costAmount < 0 || input.costAmount > 10_000_000)) {
    throw new Error('Maintenance cost must be a whole amount between ₹0 and ₹1,00,00,000.');
  }
  return { category, costAmount: input.costAmount, description, endDate: input.endDate, startDate: input.startDate, status, vendor };
}
