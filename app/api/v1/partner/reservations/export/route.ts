...
export async function GET(request: Request) {
  ...
  if (!access?.partnerId || access.partnerType !== 'CAR')
    return Response.json(
      { error: { code: 'CAR_PARTNER_REQUIRED', message: 'An active car supplier account is required.' } },
      { status: 403 },
    );
  const totalCount = await prisma.partnerVehicleReservation.count({ where: { partnerId: access.partnerId } });
  ...
  const rows: Array<Array<string | number | null>> = [[
    'Confirmation code', 'Status', 'Driver or lead traveller', 'Customer email', 'Vehicle',
    'Registration', 'Pickup location', 'Drop-off location', 'Pickup date', 'Drop-off date',
    'Units', 'Currency', 'Total amount', 'Created at',
  ]];
  for (const reservation of reservations) rows.push([
    reservation.confirmationCode, reservation.status, reservation.customerName,
    reservation.customerEmail, reservation.vehicle.vehicleName,
    reservation.vehicle.registrationNumber, reservation.vehicle.pickupLocation,
    reservation.vehicle.dropoffLocation, reservation.pickupDate, reservation.dropoffDate,
    reservation.units, reservation.currency, reservation.totalAmount,
