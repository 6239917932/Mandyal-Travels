...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    const input = normalizeBusTrip({ amenities: String(body.amenities ?? '').split(',').map((value) => value.trim()).filter(Boolean), arrivalTime: String(body.arrivalTime ?? ''), busType: String(body.busType ?? ''), cancellationPolicy: String(body.cancellationPolicy ?? ''), departureTime: String(body.departureTime ?? ''), pricePerSeat: Number(body.pricePerSeat), refundable: body.refundable === true, seatCapacity: Number(body.seatCapacity), serviceDate: String(body.serviceDate ?? '') });
