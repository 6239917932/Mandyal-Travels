...
export async function POST(request: Request, { params }: Context) {
  ...
  const route = await prisma.partnerBusRoute.findFirst({
    where: { id: routeId, partnerId: access.partnerId, status: 'ACTIVE' },
  });
  if (!route) return failure('BUS_ROUTE_NOT_FOUND', 'The active route was not found.', 404);
  const body = await readJsonObject(request);
  if (!body) return failure('INVALID_JSON', 'Enter valid trip details.', 400);
  try {
    const input = normalizeBusTrip(
      {
    ...
    const data = await prisma.partnerBusTrip.create({
      data: {
    ...
    await recordPartnerAudit(access, {
      action: 'BUS_TRIP_CREATED',
      entityId: data.id,
      entityType: 'BUS_TRIP',
      metadata: { routeId, serviceDate: data.serviceDate },
      summary: `${route.origin} to ${route.destination} trip scheduled.`,
    });
