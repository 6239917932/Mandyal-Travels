...
export async function PATCH(
  ...
  if (access.memberRole !== 'ADMIN')
    return failure('PARTNER_ADMIN_REQUIRED', 'Only the supplier administrator can manage rate plans.', 403);
  ...
  try {
    ...
    const data =
      action === 'PAUSE'
        ? await partnerOperationsService.pauseRatePlan(access.partnerId, propertyId, roomId, ratePlanId)
        : action === 'RESTORE'
          ? await partnerOperationsService.restoreRatePlan(access.partnerId, propertyId, roomId, ratePlanId)
          : action === 'UPDATE'
    ...
    const auditVerb = action === 'UPDATE' ? 'UPDATED' : action === 'RESTORE' ? 'RESTORED' : 'PAUSED';
