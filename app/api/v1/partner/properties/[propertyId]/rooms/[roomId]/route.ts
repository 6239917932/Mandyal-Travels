...
export async function PATCH(
  ...
  if (access.memberRole !== 'ADMIN')
    return failure(
      ...
      'Only the supplier administrator can edit room types.',
      403,
    );
  ...
  try {
    ...
    const data =
      action === 'PAUSE' || action === 'RESTORE'
        ? await partnerOperationsService.setRoomTypeStatus(
            access.partnerId,
            propertyId,
            roomId,
            action,
    ...
    await recordPartnerAudit(access, {
      ...
      metadata: { propertyId },
      summary: `${data.name} was ${action.toLowerCase()}d.`,
    });
    return Response.json({ data });
  } catch (error) {
    return error instanceof PartnerOperationsError
      ? failure(error.code, error.message, 409)
      : failure('ROOM_UPDATE_FAILED', 'The room type could not be updated.', 500);
  }
}
