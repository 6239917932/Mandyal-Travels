...
export async function PATCH(request: Request, { params }: Context) {
  ...
  if (access.memberRole !== 'ADMIN')
    return failure(
      ...
      'Only a supplier administrator can update compliance records.',
      403,
    );
  ...
  try {
    ...
    return error instanceof PartnerOperationsError
      ...
      : failure(
          'INVALID_VEHICLE_COMPLIANCE',
          error instanceof Error ? error.message : 'Compliance dates could not be saved.',
          400,
        );
  }
}
