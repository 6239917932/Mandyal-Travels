...
export async function POST(request: Request, { params }: Context) {
  ...
  const vehicle = await prisma.partnerVehicle.findFirst({
    where: { id: vehicleId, partnerId: access.partnerId },
  });
  ...
  try {
    const input = normalizeVehicleMaintenance({
      ...
      costAmount:
        body.costAmount === '' || body.costAmount == null ? undefined : Number(body.costAmount),
    ...
    const data = await prisma.$transaction(async (transaction) => {
      const record = await transaction.partnerVehicleMaintenance.create({
        data: { ...input, vehicleId },
      });
      if (input.status !== 'COMPLETED') {
        ...
        for (
          ...
          await transaction.partnerVehicleInventoryDay.upsert({
            create: {
              availableUnits: 0,
              note: `Maintenance: ${input.category}`,
              serviceDate,
              stopSell: true,
              vehicleId,
            },
    ...
    await recordPartnerAudit(access, {
      ...
      summary: `${vehicle.vehicleName} maintenance recorded.`,
    });
    return Response.json({ data }, { status: 201 });
  } catch (error) {
    return failure(
      'INVALID_MAINTENANCE',
      error instanceof Error ? error.message : 'Maintenance could not be recorded.',
      400,
    );
  }
}
