...
import {
  ...
  transitionPartnerHotelGuestServiceOrder,
...
export async function POST(request: Request, context: { params: Promise<{ orderId: string }> }) {
  ...
  if (!body) return failure('INVALID_SERVICE_ACTION', 'Enter a valid service action.', 400);
  const { orderId } = await context.params;
  try {
    const order = await transitionPartnerHotelGuestServiceOrder({
    ...
    return failure('SERVICE_ACTION_FAILED', 'The guest-service order could not be updated.', 500);
  }
}
