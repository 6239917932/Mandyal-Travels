...
import {
  IntegrationOutboxAutomationError,
  runIntegrationOutboxAutomation,
} from '@/services/integrationOutboxAutomationService';
...
function authorized(request: Request): boolean {
  const configured = process.env.INTEGRATION_OUTBOX_WORKER_SECRET?.trim() ?? '';
...
async function readRequest(
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new IntegrationOutboxAutomationError('INVALID_REQUEST_BODY');
  }
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    return Response.json(
      await runIntegrationOutboxAutomation({
        batchSize: body.batchSize ?? process.env.INTEGRATION_OUTBOX_WORKER_BATCH_SIZE,
        correlationId: body.correlationId ?? request.headers.get('x-correlation-id') ?? undefined,
        leaseSeconds: process.env.INTEGRATION_OUTBOX_WORKER_LEASE_SECONDS,
      }),
    );
  } catch (error) {
    ...
    if (error instanceof IntegrationOutboxAutomationError) {
      if (
        error.code === 'AUTOMATION_ALREADY_RUNNING' ||
        error.code === 'DUPLICATE_CORRELATION_ID'
      ) {
        return Response.json(
          { error: 'Integration delivery pass already running' },
          { status: 409 },
        );
      }
    ...
    return Response.json({ error: 'Integration delivery failed' }, { status: 500 });
  }
}
