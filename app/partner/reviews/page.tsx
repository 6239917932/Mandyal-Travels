...
export default async function PartnerHotelReviewsPage() {
  ...
  return (
    <section className="account-page review-governance">
      <Link className="hotel-details-page__back-link" href="/partner">
        Back to supplier workspace
      </Link>
      <header className="account-trips__heading">
        <p className="hotel-page__eyebrow">Guest feedback</p>
        <h1>Published hotel reviews</h1>
        <p>Respond professionally on behalf of the property. Responses are public and audited.</p>
      </header>
        ...
        {reviews.map((review) => (
          <Card key={review.id}>
            ...
            {review.partnerReply ? (
              ...
              <PartnerHotelReviewReply reviewId={review.id} />
            )}
          </Card>
        ))}
        {reviews.length === 0 ? (
          <Card>
            <strong>No published reviews are available for your properties.</strong>
          </Card>
        ) : null}
      </div>
    </section>
