...
export default function PartnerBusBookingsPage() {
  ...
  const load = useCallback(async (page: number) => {
    ...
    try {
      ...
      const result = await readJsonResponse<{ data: Reservation[]; meta: Meta } | ApiErrorResponse>(
        response,
      );
      if (!response.ok || !result || !('data' in result))
        throw new Error('Bus reservations could not be loaded.');
      setReservations(result.data);
      setMeta(result.meta);
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : 'The reservation service could not be reached.',
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const task = window.setTimeout(() => void load(1), 0);
  ...
  return (
    <section className="account-page partner-bookings">
      <div className="partner-page__heading">
        <div>
          <p className="hotel-page__eyebrow">Bus operator reservations</p>
          <h1>Passenger and seat manifest</h1>
          <p>
            Review confirmed direct-channel passengers, seats, routes, services, and captured value.
          </p>
        </div>
        <div className="manage-booking__document-actions">
          <a
            className="ui-button ui-button--secondary"
            href="/api/v1/partner/bus-reservations/export"
          >
            Export CSV
          </a>
      ...
      {meta ? (
        <>
          ...
          <Card className="partner-bookings__search">
            <Input
              ...
              placeholder="Reference, passenger, email, origin, or destination"
              value={filter}
            />
          </Card>
          <div className="partner-bookings__list" aria-live="polite">
              ...
              <Card className="partner-bookings__booking" key={reservation.confirmationCode}>
                ...
                <div className="booking-confirmation__details">
                  ...
                  <div>
                    <span>Service</span>
                    <strong>
                      {reservation.trip.serviceDate} · {reservation.trip.departureTime}
                    </strong>
                    <small>{reservation.trip.busType}</small>
                  </div>
                  <div>
                    <span>Passengers</span>
                    <strong>{reservation.passengerCount}</strong>
                  </div>
                  <div>
                    <span>Seats</span>
                    <strong>{seats(reservation.seatNumbersJson) || 'Not assigned'}</strong>
                  </div>
                  <div>
                    <span>Total</span>
            ...
            {!filtered.length ? <Card>No bus reservations match this view.</Card> : null}
          </div>
        ...
        <Card>Loading bus reservations…</Card>
      ) : null}
    </section>
  );
}
