...
test('integration retries use bounded exponential backoff', () => {
  ...
  assert.equal(outboxRetryDecision({ attempts: 0, maxAttempts: 8, now }).nextAttemptAt.toISOString(), '2026-08-15T10:01:00.000Z');
  assert.equal(outboxRetryDecision({ attempts: 7, maxAttempts: 20, now }).nextAttemptAt.toISOString(), '2026-08-15T12:08:00.000Z');
  assert.equal(outboxRetryDecision({ attempts: 15, maxAttempts: 20, now }).nextAttemptAt.toISOString(), '2026-08-15T16:00:00.000Z');
});
