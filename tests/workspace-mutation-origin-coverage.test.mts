...
test('every business and agent mutation route enforces same-origin browser requests', async () => {
  const missing: string[] = [];
  for (const workspace of ['business', 'agent']) {
    for (const routePath of await routeFiles(path.resolve(`app/api/v1/${workspace}`))) {
