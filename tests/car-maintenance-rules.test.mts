...
test('vehicle maintenance normalizes bounded operational records', () => {
  assert.deepEqual(normalizeVehicleMaintenance({
...
test('vehicle maintenance rejects reversed dates and unsafe costs', () => {
  const base = { category: 'Repair', description: 'Brake repair required.', endDate: '2026-09-01', startDate: '2026-09-02', status: 'IN_PROGRESS' };
  assert.throws(() => normalizeVehicleMaintenance(base));
  assert.throws(() => normalizeVehicleMaintenance({ ...base, costAmount: 10_000_001, endDate: '2026-09-03' }));
});
