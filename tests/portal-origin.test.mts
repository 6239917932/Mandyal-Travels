...
test('portal mutations accept the public host when a reverse proxy uses an internal request URL', () => {
  const request = new Request(
    'https://mandyal-travels-production.up.railway.app/api/v1/auth/logout',
    {
      method: 'POST',
      headers: {
...
test('production logout redirects through the configured public origin', async () => {
  const logout = await readFile(
    new URL('../app/api/v1/auth/logout/route.ts', import.meta.url),
    'utf8',
  );
